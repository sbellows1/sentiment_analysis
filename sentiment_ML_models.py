...
def build_train_model(sequence, y, id_dict, embedding_size,
    lstm_size, dropout, epochs, batch_size, filename, figname):
    model = Sequential()
...
def test_metric_rnn(rnn, test, test_labels):
    return f1_score(test_labels, pd.Series(rnn.predict(test).flatten()).map(round))
...
if __name__ == "__main__":
    ...
    model_df = pd.DataFrame({'model': ['MultinomialNB', 'GaussianNB', 'GradientBoosting'],
        'metric': [m['test_score'][0] for m in trained_models]})
